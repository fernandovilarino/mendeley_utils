...
def list_documents():
    ...
    # pprint(groups)
    #g_id = "40dfc949-a6d7-302f-be1d-271c45b3b5a6"
    g_id = "61f3bc61-e2f3-36fd-9327-53146195c74f"
    p_size = 500
    ...
    createCSV(docs, authors, sours, links, count)
    df = pd.read_csv('./templates/fer_library_csv_sorted.csv', sep='\t')
    ind_sorted = df.values[:,0]
    #pprint("------&&&&&&--------")
    #pprint(ind_sorted)

    createHTML(docs, authors, sours, links, count, ind_sorted)


    #return render_template('library.html', name=name, docs=docs, auths=authors, sours=sours, links=links, count = count)
...
def createHTML(docs, auths, sours, links, count, ordered_indexes):
    style_IEEE = {
        "authors":  {"data": '{}',
    ...
    filename = './templates/fer_library_html.html'
...
def createCSV(docs, auths, sours, links, count, incl_lastname = 1):
